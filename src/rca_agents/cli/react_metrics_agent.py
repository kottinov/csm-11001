"""CLI for React metrics analysis agent (baseline)."""
...
from ..react_agents.react_metrics_agent import (
    DEFAULT_METRICS_QUESTION,
    build_graph,
    default_question,
    run as run_react_metrics_agent,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the React metrics RCA agent (baseline)."
    )
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    result = run_react_metrics_agent(question, graph)
    print(result)
